...
export function generateReportV2(runResult: VerifyResult, runDir: string, options: ReportOptions = {}): string {
  ...
  const html = buildHtmlV2(runResult, screenshotData);
  writeFileSync(outputPath, html);
  return outputPath;
}

export function buildHtmlV2(run: VerifyResult, screenshots: Map<string, string> = new Map()): string {
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .steps { max-width: 800px; margin: 0 auto; display: flex; flex-direction: column; gap: 12px; }
  ...
  @media (max-width: 600px) { body { padding: 12px; } .step-screenshot { max-width: 100%; } }
</style>
</head>
<body>
<div class="header">
