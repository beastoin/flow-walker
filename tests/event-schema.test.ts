...
describe('EVENT_TYPES', () => {
  it('has 9 types', () => { assert.equal(EVENT_TYPES.length, 9); });
  ...
  it('includes action, assert, artifact, note, agent-review', () => { for (const t of ['action', 'assert', 'artifact', 'note', 'agent-review']) assert.ok(EVENT_TYPES.includes(t as any)); });
});
describe('validateEvent', () => {
  ...
  it('accepts agent-review with step_id', () => { assert.ok(validateEvent({ type: 'agent-review', step_id: 'S1', prompt_idx: 0, verdict: 'pass' }).valid); });
  it('rejects agent-review without step_id', () => { assert.ok(!validateEvent({ type: 'agent-review' }).valid); });
